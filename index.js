...
** "persistentFilename"
*/
let persistFilename = "untitled.html";
...
ipcRenderer.on('newly-made-file', function (event, path) {
  ...
  tinymce.activeEditor.resetContent(initialContent);
...
tinymce.init({
  ...
  */
  toolbar: 'file undo redo styleselect bold italic strikethrough bullist numlist link blockquote underline hr table image preview searchreplace code fullscreen superscript subscript', // 
  ...
  */
  plugins: 'code codesample, link image table lists paste autolink hr quickbars preview searchreplace fullpage', // More: charmap, emoticons
...
window.addEventListener('keydown', function (event) {
  ...
  if (!event.shiftKey && (event.ctrlKey || event.metaKey) && event.code === 'KeyS') {
    ...
    // blocks the upload of <img> elements with the attribute "internal-blob".
    saveFile(false);
